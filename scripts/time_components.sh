...
# Raw `time` logs are appended to `timings.log`. Takes about ten minutes. Portable to bash 3
...
for m in Functions Embeddings Isomorphisms Naturals Recursion Integers Booleans Sum Quotient \
  ...
  for i in 1 2 3; do
    ...
    echo "$m run$i $t" >> timings.log
    times="$times$t
"
  done
  med=$(printf "%s" "$times" | sort -n | sed -n 2p)
  echo "$m median $med" >> timings.log
  printf "%-16s median %6.1f s\n" "$m" "$med"
  c=$(component "$m")
  SUM[$c]=$(echo "${SUM[$c]} + $med" | bc)
done
rm -f .time.tmp
echo "--- per component (sum of module medians) ---"
total=0
